...
void bluefruitle_task_init(void)
{
    battery_init();
}

void bluefruitle_task(void)
{
#if 0
    int16_t c;
    // Raw mode: interpret output report of LED state
    while ((c = bluefruitle_getc()) != -1) {
    ...
    if (!config_mode && !force_usb) {
        if (!bluefruitle_rts() && host_get_driver() != &bluefruitle_driver) {
            clear_keyboard();
            host_set_driver(&bluefruitle_driver);
        } else if (bluefruitle_rts() && host_get_driver() != &lufa_driver) {
            clear_keyboard();
    ...
    /* Connection monitor */
    if (!bluefruitle_rts() && bluefruitle_linked()) {
        status_led(true);
    } else {
        status_led(false);
    }
 ...
 * Command
 ******************************************************************************/
static host_driver_t *prev_driver = &bluefruitle_driver;

static void print_bluefruitle(void)
{
    int16_t c;
    while ((c = bluefruitle_getc()) != -1) {
        xprintf("%c", c);
...
static void clear_bluefruitle(void)
{
    while (bluefruitle_getc() != -1) ;
}
...
static void enter_command_mode(void)
{
    ...
    clear_keyboard();
    host_set_driver(&bluefruitle_config_driver);   // null driver; not to send a key to host
    bluefruitle_disconnect();
    while (bluefruitle_linked()) ;
...
static void init_bluefruitle(void)
{
#if 0
    // Bluefruit LE configure
...
#define BLUEBRUITLE_LINK0  (uint8_t *)128
#define BLUEBRUITLE_LINK1  (uint8_t *)140
#define BLUEBRUITLE_LINK2  (uint8_t *)152
#define BLUEBRUITLE_LINK3  (uint8_t *)164
static void store_link(uint8_t *eeaddr)
{
    enter_command_mode();
    SEND_STR("GR\r\n"); // remote address
    const char *s = bluefruitle_gets(500);
    if (strcmp("GR", s) == 0) s = bluefruitle_gets(500);   // ignore local echo
    xprintf("%s(%d)\r\n", s, strlen(s));
    if (strlen(s) == 12) {
        for (int i = 0; i < 12; i++) {
...
bool command_extra(uint8_t code)
{
    ...
    switch (code) {
        ...
        case KC_SLASH: /* ? */
            ...
            print("Del:     enter/exit config mode\n");
            print("Slck:    initialize\n");
#if 0
        ...
        case KC_F1:
            store_link(BLUEFRUITLE_LINK0);
            return true;
        case KC_F2:
            store_link(BLUEFRUITLE_LINK1);
            return true;
        case KC_F3:
            store_link(BLUEFRUITLE_LINK2);
            return true;
        case KC_F4:
            store_link(BLUEFRUITLE_LINK3);
            return true;
        /* Restore link address to EEPROM */
        case KC_1:
            restore_link(BLUEFRUITLE_LINK0);
            return true;
        case KC_2:
            restore_link(BLUEFRUITLE_LINK1);
            return true;
        case KC_3:
            restore_link(BLUEFRUITLE_LINK2);
            return true;
        case KC_4:
            restore_link(BLUEFRUITLE_LINK3);
            return true;
#endif
        case KC_I:
            print("\n----- Bluefruit LE info -----\n");
            xprintf("protocol: %s\n", (host_get_driver() == &bluefruitle_driver) ? "BLE" : "LUFA");
            xprintf("force_usb: %X\n", force_usb);
            xprintf("bluefruitle: %s\n", bluefruitle_rts() ? "OFF" : (bluefruitle_linked() ? "CONN" : "ON"));
            xprintf("bluefruitle_autoconnecting(): %X\n", bluefruitle_autoconnecting());
...
#if 0
            xprintf("LINK0: %s\r\n", get_link(BLUEFRUITLE_LINK0));
            xprintf("LINK1: %s\r\n", get_link(BLUEFRUITLE_LINK1));
            xprintf("LINK2: %s\r\n", get_link(BLUEFRUITLE_LINK2));
            xprintf("LINK3: %s\r\n", get_link(BLUEFRUITLE_LINK3));
#endif
            return true;
        case KC_B:
            // battery monitor
        ...
        case KC_U:
            ...
            return true;
        case KC_DELETE:
            /* Bluefruit LE Command mode */
            if (bluefruitle_autoconnecting()) {
                enter_command_mode();
...
{
    bluefruitle_putc(code2asc(code));
    return true;
}
